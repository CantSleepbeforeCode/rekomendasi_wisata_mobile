...
class _WisataPageState extends State<WisataPage> with TickerProviderStateMixin {
  final Box _boxLogin = Hive.box("login");

  TextEditingController controller = new TextEditingController();
  List<Widget> imageSliders = [];
  List<WisataModel> carouselImage = [];
  bool isLoaded = false;
        ...
        child: BlocBuilder<AppCubits, CubitStates>(
          builder: (context, state) {
            if (state is LoadedState) {
              ...
              if (!isLoaded) {
                _searchResult = wisatas;

                if (wisatas.length >= 3) {
                  for (var i = 0; i <= 2; i++) {
                    carouselImage.add(wisatas[i]);
                  }
                } else {
                ...
                imageSliders = carouselImage
                    .map((item) => GestureDetector(
                          onTap: () {
                            ...
                            //         double.parse(item.wisata_latitude),
                            //         double.parse(item.wisata_longitude));
                            BlocProvider.of<AppCubits>(context)
                                .detailDataWisata(item);
                          },
                          child: Container(
                            child: Container(
                              ...
                              child: ClipRRect(
                                  ...
                                  child: Stack(
                                    children: <Widget>[
                                      Image.network(
                                          Env.urlAsset + item.wisata_picture,
                                          fit: BoxFit.cover,
                                          width: 1000.0),
                                      Positioned(
                                        bottom: 0.0,
  ...
  onSearchTextChanged(String text) async {
    ...
    if (text.isEmpty) {
      ...
      wisatas.forEach((wisata) {
        String wisataName = wisata.wisata_name.toLowerCase();
        if (wisataName.contains(text.toLowerCase())) {
...
List<WisataModel> _searchResult = [];
